...
export async function seedMockDataIfNeeded(deps: SeedDeps): Promise<void> {
  ...
  if (!hasMenuScan) {
    ...
    const menuResult: MenuScanResult = {
      ...
      caution: [{ name: 'Pasta carbonara', shortReason: 'Energy dense, portion matters.', pins: ['Lower sodium', 'Portion control', 'Fiber'], confidencePercent: 65, dietBadges: [], allergenNote: null, noLine: null }],
      avoid: [{ name: 'Fried combo set', shortReason: 'High calories and low satiety.', pins: ['Lower calorie', 'Portion control', 'Whole foods'], confidencePercent: 90, dietBadges: [], allergenNote: null, noLine: null }],
